...
### 新增
- 「最近决策」面板新增周期筛选：可切换「全部周期 / 仅交易决策」，仅显示 AI 实际执行开仓/平仓动作的周期（服务端过滤，最多回溯最近 500 条决策记录）
...
### 修复
- 自定义 System Prompt 在实盘交易中不生效的问题：交易员自定义提示词此前仅保存至数据库、从未拼入实盘决策的 System Prompt，现已正确接入策略引擎（追加模式，附加于系统提示词末尾的「Personalized Trading Strategy」段）
- Prompt Lab「提示词变体表现」面板百分比显示错误：总收益率、胜率、最大回撤此前被重复 ×100（如 5% 显示为 500%），现已与变体列表卡片口径一致
