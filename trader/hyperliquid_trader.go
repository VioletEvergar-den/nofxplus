...
type HyperliquidTrader struct {
	...
	privateKey    *ecdsa.PrivateKey // For order signing
	isTestnet     bool
}
...
func (t *HyperliquidTrader) GetBalance() (map[string]interface{}, error) {
	...
	totalUnrealizedPnlAll := totalUnrealizedPnl

	// Calculate total equity properly: perpAccountValue + spotUSDCBalance
	totalEquityCalculated := accountValue + spotUSDCBalance

	result["totalWalletBalance"] = totalWalletBalance       // Total assets (Perp + Spot) - unrealized
	result["totalEquity"] = totalEquityCalculated           // Total equity = Perp AV + Spot
	result["availableBalance"] = availableBalance           // Available balance (Perpetuals only)
	result["totalUnrealizedProfit"] = totalUnrealizedPnlAll // Unrealized PnL (Perpetuals)
	...
	logger.Infof("  ⭐ Total: %.2f USDC | Perp: %.2f | Spot: %.2f",
