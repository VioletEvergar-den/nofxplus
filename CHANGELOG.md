...
### Added
- Cycle filter for the Latest Decisions panel: switch between "All Cycles" and "Trades Only" to show only cycles where the AI actually executed open/close actions (server-side filtering, scans up to the latest 500 decision records)
...
### Fixed
- Custom System Prompt not taking effect in live trading: trader-level custom prompts were previously only saved to the database and never included in the live decision System Prompt; they are now correctly wired into the strategy engine (append mode, added as a "Personalized Trading Strategy" section at the end of the system prompt)
- Incorrect percentage display in Prompt Lab "Prompt Variant Performance" panel: total return, win rate, and max drawdown were multiplied by 100 twice (e.g., 5% shown as 500%); now consistent with the variant list cards
